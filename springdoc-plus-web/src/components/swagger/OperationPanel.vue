...
const headerColor = computed(() => methodHeaderColors[props.item.method] ?? 'border-[var(--c-border)] bg-white')
...
<template>
  <div :id="opId" class="mb-4 overflow-hidden rounded-2xl border border-[var(--c-border)] bg-white shadow-[0_2px_12px_rgb(0_0_0/6%)]">
    <!-- Header -->
    <div
      class="flex flex-wrap items-center gap-3 border-b px-5 py-3.5"
      :class="headerColor"
    >
    ...
    <div class="flex min-h-0 flex-col xl:flex-row">
      ...
      <div class="min-w-0 flex-1 divide-y divide-[var(--c-border)]">
        ...
        <div v-if="parameters.length" class="px-5 py-4">
          <h4 class="mb-3 flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-[var(--c-muted)]">
            <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M4 6h16M4 12h16M4 18h7" /></svg>
            请求参数
          </h4>
            ...
            <table class="w-full border-collapse text-[12px]">
              <thead>
                <tr class="border-b border-[var(--c-border)] bg-[var(--c-bg)]">
                  <th class="px-3 py-2.5 text-left text-[10px] font-bold uppercase tracking-wider text-[var(--c-muted)]">名称</th>
                  <th class="px-3 py-2.5 text-left text-[10px] font-bold uppercase tracking-wider text-[var(--c-muted)]">位置</th>
                  <th class="px-3 py-2.5 text-left text-[10px] font-bold uppercase tracking-wider text-[var(--c-muted)]">类型</th>
                  <th class="px-3 py-2.5 text-left text-[10px] font-bold uppercase tracking-wider text-[var(--c-muted)]">必填</th>
                  <th class="px-3 py-2.5 text-left text-[10px] font-bold uppercase tracking-wider text-[var(--c-muted)]">说明</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-[var(--c-border)]">
                ...
                >
                  <td class="px-3 py-2.5 font-mono text-[12px] font-semibold text-[var(--c-text)]">{{ p.name }}</td>
                  ...
                  <td class="px-3 py-2.5 font-mono text-[11px] text-[var(--c-muted)]">{{ p.schema?.type ?? '?' }}</td>
                  <td class="px-3 py-2.5">
                    <span v-if="p.required" class="rounded-full bg-rose-50 px-2 py-0.5 text-[10px] font-bold text-rose-600">必填</span>
                    <span v-else class="text-[11px] text-[var(--c-border)]">—</span>
                  </td>
                  <td class="px-3 py-2.5 text-[11px] text-[var(--c-muted)]">{{ p.description ?? '—' }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div v-if="requestBodyContent" class="px-5 py-4">
          <h4 class="mb-3 flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-[var(--c-muted)]">
            ...
            请求体 Schema
            <span v-if="op.requestBody?.required" class="ml-0.5 rounded-full bg-rose-50 px-2 py-0.5 text-[9px] font-bold normal-case text-rose-600">必填</span>
          </h4>
          <div class="space-y-3">
            <div v-for="(media, contentType) in requestBodyContent" :key="contentType">
              <span class="mb-2 inline-block rounded-md border border-[var(--c-border)] bg-[var(--c-bg)] px-2 py-0.5 font-mono text-[10px] text-[var(--c-muted)]">
                {{ contentType }}
              </span>
        ...
        <div v-if="responses.length" class="px-5 py-4">
          <div class="mb-3 flex items-center gap-3">
            <h4 class="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-[var(--c-muted)]">
              <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round">
            ...
            >
              <svg class="h-3.5 w-3.5 transition-transform" :class="responseDocsCollapsed ? '-rotate-90' : 'rotate-0'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                <path d="m6 9 6 6 6-6" />
              </svg>
            </button>
          </div>
          ...
          <div v-if="!responseDocsCollapsed" class="grid gap-2.5">
            ...
            >
              <div class="flex flex-wrap items-center gap-2 border-b border-[var(--c-border)] bg-[var(--c-bg)] px-4 py-2.5">
                <span class="rounded-full border px-2.5 py-0.5 font-mono text-[11px] font-bold" :class="statusColor(code)">
                  {{ code }}
                </span>
              ...
              <div v-if="resp.content" class="space-y-3 px-4 py-3">
                ...
                >
                  <span class="mb-2 inline-flex rounded-full border border-[var(--c-border)] bg-white px-2.5 py-0.5 font-mono text-[10px] text-[var(--c-muted)]">
                    {{ ct }}
                  </span>
      ...
      <div class="shrink-0 border-t border-[var(--c-border)] bg-[var(--c-bg)/30] xl:w-[440px] xl:border-l xl:border-t-0">
