...
const availableContentTypes = computed(() =>
  getRequestBodyOptions(props.item).map((option) => option.effectiveType),
)
const requestBodyOption = computed(() =>
  resolveRequestBodyOption(props.item, simulate.contentType.value),
)
...
const schemaFields = computed<SchemaFieldMeta[]>(() => {
  ...
  return Object.entries(resolved.properties).map(([name, fieldSchema]) => {
    ...
    const fieldType = getSchemaPrimaryType(field) ?? 'string'
    const isArray = fieldType === 'array'
    return {
      name,
      type: isArray ? 'array' : fieldType,
      isArray,
...
function restoreUiSnapshot(key: string) {
  ...
  activeTab.value = tabs.value.some((t) => t.key === snapshot.activeTab)
    ? snapshot.activeTab
    : getDefaultTab()
...
function saveUiSnapshot() {
  ...
  writeStorage(uiStorageKey(operationKey.value), snapshot)
}
...
function handleReset() {
  ...
  removeStorage(uiStorageKey(operationKey.value))
  customHeaders.value = []
  formFieldValues.value = {}
  formFileValues.value = {}
  activeTab.value = getDefaultTab()
...
function buildFormData(): FormData {
  ...
  for (const field of schemaFields.value) {
    if (field.isBinary) {
      ...
      if (field.isArray) {
        for (const file of files) formData.append(field.name, file)
      } else formData.append(field.name, files[0])
      continue
    }
    const value = formFieldValues.value[field.name]
    if (value !== undefined && value !== '') formData.append(field.name, value)
  }
  return formData
}
...
function buildFormUrlEncodedBody(): string {
  ...
  for (const [name, value] of Object.entries(formFieldValues.value)) {
    if (value !== undefined && value !== '') sp.append(name, value)
  }
  return sp.toString()
}
...
watch(hasRequestBodyEditor, (enabled) => {
  if (!enabled && activeTab.value === 'body') activeTab.value = 'params'
})
...
<template>
  <div class="flex h-full flex-col">
    <!-- Panel Header -->
    <div
      class="flex items-center gap-2.5 border-b border-[var(--c-border)] bg-[var(--c-bg)] px-4 py-3"
    >
      <div
        class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[var(--c-primary)] text-white shadow-sm"
      >
        <svg
          class="h-3.5 w-3.5"
          viewBox="0 0 24 24"
    ...
    <div v-if="!collapsed" class="flex flex-1 flex-col gap-0">
      ...
      <div class="flex-1 px-4 py-3">
        ...
        <div v-show="activeTab === 'params'">
          <div v-if="simulate.params.value.length" class="space-y-2">
            ...
            >
              <div class="mb-2 flex flex-wrap items-center gap-1.5">
                <span class="font-mono text-[12px] font-semibold text-[var(--c-text)]">
                  {{ param.name }}
                </span>
                <span
                  class="rounded-md px-1.5 py-0.5 text-[10px] font-semibold"
                  :class="getParamLocationClass(param.in)"
                >
                  {{ param.in }}
                </span>
                <span
                ...
                <span
                  v-if="param.type"
                  class="ml-auto rounded bg-[var(--c-bg)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--c-muted)]"
                >
                  {{ param.type }}
                </span>
              </div>
              <p v-if="param.description" class="mb-1.5 text-[11px] text-[var(--c-muted)]">
                {{ param.description }}
              </p>
              <input
                v-model="param.value"
          ...
          <p
            v-else
            class="rounded-xl border border-dashed border-[var(--c-border)] px-3 py-6 text-center text-[12px] text-[var(--c-muted)]"
          >
            当前接口没有请求参数
          </p>
        </div>
        ...
        <div v-show="activeTab === 'headers'">
          ...
          <div v-if="customHeaders.length" class="space-y-2">
            <div v-for="(header, index) in customHeaders" :key="index" class="flex gap-2">
              <input
                v-model="header.name"
        ...
        <div v-show="activeTab === 'body' && hasRequestBodyEditor">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <div class="flex items-center gap-2">
              ...
              >
                <option v-for="type in availableContentTypes" :key="type" :value="type">
                  {{ type }}
                </option>
              </select>
              <span
          ...
          <p
            v-if="schemaHint"
            class="mb-3 rounded-lg border border-[var(--c-border)] bg-[var(--c-bg)] px-3 py-2 text-[11px] leading-5 text-[var(--c-muted)]"
          >
            {{ schemaHint }}
          </p>
          <p
            v-if="isForm && schemaFields.some((f) => f.isBinary)"
            class="mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] leading-5 text-amber-700"
          >
          ...
          <div v-if="isForm && schemaFields.length" class="space-y-2">
            ...
            >
              <div class="mb-2 flex flex-wrap items-center gap-1.5">
                ...
                <span
                  class="rounded bg-[var(--c-bg)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--c-muted)]"
                >
              ...
              <p v-if="field.description" class="mb-1.5 text-[11px] text-[var(--c-muted)]">
                {{ field.description }}
              </p>
              ...
              <input
                ...
                class="block w-full rounded-lg border border-[var(--c-border)] bg-[var(--c-bg)] px-3 py-1.5 text-[12px] file:mr-2 file:rounded-full file:border-0 file:bg-[var(--c-primary-light)] file:px-2.5 file:py-1 file:text-[11px] file:font-medium file:text-[var(--c-primary)]"
                @change="handleFileChange(field.name, $event)"
              />
              <div
                v-if="field.isBinary && formFileValues[field.name]?.length"
                class="mt-1 rounded-lg bg-[var(--c-bg)] px-3 py-1.5 text-[11px] text-[var(--c-muted)]"
              >
                {{ formFileValues[field.name].map((f) => f.name).join(' , ') }}
              </div>
              <select
                v-else-if="field.enum?.length"
                v-model="formFieldValues[field.name]"
                class="w-full rounded-lg border border-[var(--c-border)] bg-white px-3 py-1.5 text-[12px] text-[var(--c-text)] outline-none focus:border-[var(--c-primary)]"
              >
              ...
              <input
                v-else
                v-model="formFieldValues[field.name]"
                class="w-full rounded-lg border border-[var(--c-border)] bg-[var(--c-bg)] px-3 py-1.5 text-[12px] text-[var(--c-text)] outline-none transition-colors focus:border-[var(--c-primary)] focus:bg-white"
                :placeholder="field.example || field.type || '请输入字段值'"
              />
              <p v-if="field.example" class="mt-1 text-[10px] text-[var(--c-muted)]">
                示例：{{ field.example }}
              </p>
            </div>
          </div>
          ...
          <div v-else class="space-y-3">
            <div v-if="schemaFields.length" class="grid grid-cols-2 gap-1.5">
              <div
                v-for="field in schemaFields"
                :key="field.name"
                class="rounded-lg border border-[var(--c-border)] bg-[var(--c-bg)] px-2.5 py-2"
              >
                <div class="flex flex-wrap items-center gap-1">
                  <span class="font-mono text-[11px] font-semibold text-[var(--c-text)]">
                    {{ field.name }}
                  </span>
                  <span
                    class="rounded bg-white px-1 py-0.5 font-mono text-[9px] text-[var(--c-muted)]"
                  >
                    {{ field.type }}
                  </span>
                  <span
                    v-if="field.required"
                    class="rounded-full bg-rose-50 px-1 py-0.5 text-[9px] font-bold text-rose-600"
                  >
                    *
                  </span>
                </div>
                <p
                  ...
                  class="mt-0.5 line-clamp-1 text-[10px] text-[var(--c-muted)]"
                >
                  {{ field.description }}
                </p>
              </div>
        ...
        <div v-show="activeTab === 'response'">
          <SimulateResponsePanel :result="simulate.result.value" :error="simulate.error.value" />
          <p
            v-if="!simulate.result.value && !simulate.error.value"
            class="rounded-xl border border-dashed border-[var(--c-border)] px-3 py-6 text-center text-[12px] text-[var(--c-muted)]"
          >
            点击「发送请求」后响应结果会显示在这里
          </p>
